...
- **Push only what the agent reads**: before a push the target agent is read with `POST /flow/ai-agent-info` and the Brief shows only the fields whose tokens its prompt carries; a field it does not read gets the plain warning "your bot does not use this yet" and nothing is sent. After the push **both halves are read back** — the field's value matches and the token is still in the agent's prompt — and only then is the sync recorded as sent (`faq_syncs`, with the snapshot the next Brief diffs against).
- **One budget**: `FAQ_FIELD_BUDGET` (20,000 characters until the platform's real maximum is confirmed), named on the Brief. Entries are ranked by Times Asked when the source carried it, else recency; past the budget the lowest-ranked are dropped **whole** and listed, so a cut never lands mid-sentence in the bot's prompt.
