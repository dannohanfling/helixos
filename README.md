...
| **Coach** | Client roster with today's status, streak, tier, pathway progress; verify queue; who needs a nudge | 🎓 Client Success Path, 🛣️ Success Pathway |
...
npx tsx scripts/smoke.ts    # Playwright walkthrough against a running dev server; writes ./screenshots
```
...
- `src/lib/engine/` — pure rules: `streak.ts`, `points.ts`, `tiers.ts`, `nba.ts`. Tested in `__tests__`.
...
1. Coach opens **Settings** and copies the client invite link.
