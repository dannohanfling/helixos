...
| **Integrations** (coach) | WalletPush (the Evolve Omega pass: loyalty host URL, app key, multiplier; the points and push-message client, whose call is still the unverified 6 September placeholder), Community Loyalty (uChat, inbound only: its bot flows call the webhook); `scripts/points-push-report.ts` reads the sync log and says whether a points push has ever returned success, per path and the Omnichannel Marketing System (GoHighLevel: on/off and API base only, no agency credential), inbound webhook URL + secret, sync log, per-client connection status. Points earned in HelixOS push to each member's Evolve Omega pass; inbound rewards and bookings flow back | ⚙️ Integrations |
