...
| **Webinars** | 7-step wizard: foundation → 3 belief breaks → 20-section script (5 acts) with story/analogy/objection banks → offer mapping → the deck (`src/lib/engine/deck.ts`, one read of the resolver, one key point per slide under its section's eyebrow: the Proof Block slide is the bank's proof as the bank stores it, else the citation, else no slide; the Case Study the story or nothing; the Offer Stack the linked stack as a build, one slide per item with the running total re-shown after each and the price against the total last (the anchor is the control; a total renders only when every item carries a value, so it is a real sum or absent), then the payment plan, the guarantee, and the scarcity and urgency lines only when the offer carries them; a divider on each belief act with its shift and a recap of the act's first lines, the cover and dividers on the kit's inverse pair; the offer's one-line footer on every slide from the Offer Stack onward; after the opening act's first section, who it is for and not for read off the linked Offer and the stay-to-the-end line off Foundation, and the origin story's filled beats (eight on Foundation, `ORIGIN_BEATS`) as the Credibility / Origin section's own slides; a section marked to build keeps its first point as the line and adds each further point beneath it, one slide per point; a section left out on purpose absent; a readout of slides a minute over the minutes without Q&A against the reference pace of 1.7 and the band 1.2 to 1.5, per act, which is also the twelfth build check (a refused export is a must, a pace under the band a warning); the art direction and the delivery note in the speaker notes, never on a face; and nothing about how the deck was built on a face either — consent, verification, a missing item, an internal rule or a field label, one list in `src/lib/engine/deck-face.ts`, is taken off whatever line the record hands a slide (only the clause that trips, so a citation keeps its words and loses "verified verbatim"), kept in that slide's speaker notes and listed on the Deck step, a slide that was nothing but such text is not a slide and the deck is renumbered, and a recap quotes each section's first line that can stand on a face; one currency per deck, the linked offer's own field formatted in one place (`formatPrice`), a line the coach typed naming another currency (or a dollar sign on a pound or euro offer) refusing the export with the slide named, never rewritten; a headline steps down a tier as it lengthens, 48 characters at 40pt, 80 at 32pt, 120 at 26pt, 160 at 22pt, and past 160 the sentence moves whole to the body with the section name standing in and the outline saying so; a bracketed placeholder in a sentence carrying a number, a percentage or a currency, or anywhere on a Proof Block or Offer Stack slide, refuses the export on the Deck step before the click and at the route with a 409, every other placeholder warns with the count; the workspace's brand kit on the file with its hex written verbatim, the accent drawing the rule under the eyebrow and never letters, an unfilled slot drawn in the kit's placeholder colour, and a pair that cannot read refusing the render with the pair named; the title slide, the file's author and the script's first person are the presenter, a Foundation field defaulting to the subject's own name; the file's company is the workspace and its subject the webinar, never the generator) → a run sheet (`/webinars/[id]/runsheet`, the whole webinar in running order with a cumulative clock, big type, printable: the proof, study and story wired to each act rendered in place as the bank stores them, the offer on the closing frame, its objections under Q&A, a per-section delivery note that also lands in the deck's speaker notes, unfilled `[placeholders]` counted and marked, the count naming the ones in scripts the deck does not show since the deck refuses only on what it renders; one resolver, `src/lib/engine/webinar-context.ts`, that the deck and the readiness grades read too) → readiness: an itemised build check read off the record (foundation, three belief shifts, a proof, a story and a citation wired to every act, said per act, every section scripted, no script introducing anyone but the presenter, offer linked, stack mapped, runtime, offer start time, scripts against their slots at 130 words a minute; `src/lib/engine/webinar.ts`) beside the coach's 11-point rating, of which proof sufficiency, story placement and offer clarity are read off the record with their working shown and can be lowered with a reason, never raised; "ready" needs both and the rating alone never sets it, the header names the open checks instead of a percentage, a review older than the last edit is marked stale, a ready or scheduled webinar whose checks have since broken (a proof's approval withdrawn, a study deleted, a story removed, the linked offer deleted: a reference is not a presence) is marked so in the wizard header, on the list and on Today rather than demoted, and choosing ready on Run it with a check open is refused with the checks named → run + debrief numbers. Ships with "The Leaky Webinar" as a worked example. | 🖥️ WebinarOS: 🎭 Acts, 📑 Webinar Sections, 🪜 Wizard Stages, 📖 Story Bank, 🎨 Analogies, 🗣️ Objections, 💡 Beliefs, ✅ Readiness Reviews, 🖼️ Slide Blueprints |
...
| **Socrates Domain** | The sales method, as a section of its own: the seven Foundations lessons in order (lessons, not tasks: nothing ticked, scored or counted), the question library (45, tagged by CLARITY beat, NEPQ category and script type, filterable by beat and type; a client's own questions are theirs and survive every library update), the reframe library grouped by objection (Clarify → Discuss → Diffuse), and the script wizard: a name, a script type, seven beats, each the question (a pick from the library filtered by that beat and type) with up to two follow-ups, the client's own "listen for" note, branches if the prospect pushes back (spoken reframes under their objection's condition, on by default at Tailor and Your Next Step, off elsewhere, never a principle) or the client's own words, `n of 7`; then the blanks the chosen questions carry, asked once each (`[X]` once, `[3 pillars]` prefilled from the offer's path steps); then two outputs chosen explicitly: the two-sided call sheet (YOU, listen for, branches, writing space; its own chrome-free page for a screen or a printer; copied in both clipboard flavours, the HTML built from block elements so no destination turns a break into a backslash; an empty blank shows as unfilled, never as text; the prospect's lines are never written) and the copy block (questions only, for a DM or an email). Objection scripts also pull reframes by group at Tailor. Conversations links here as CLARITY in short form. Danno's attribution line is the foot of every page in the section. Seed content in `src/data/seed/socrates/`; the question library is upserted on every migrate | Socrates OS |
...
| **Integrations** (coach) | Community Loyalty (uChat, inbound only: its bot flows call the webhook) and the Omnichannel Marketing System (GoHighLevel: on/off and API base only, no agency credential), inbound webhook URL + secret, sync log, per-client connection status. The pass provider (`walletpush` in code: points and push messages on each member's Evolve Omega wallet pass, a WalletPush instance at the loyalty host behind the Community Loyalty Mini-App) is not shown to coaches: Evolve Omega sets it up and holds its credentials, and no vendor name under Community Loyalty appears in coach-facing copy. Its call is the unverified 6 September placeholder, so `send()` refuses everything for that provider (push, ping, and never the app key) with a `skipped` row until the real call is built; its built-in host allowlist is empty because no pass host has ever answered 2xx from this codebase (the host is a default to confirm, never a constant). `scripts/points-push-report.ts` reads the sync log and says whether a points push has ever returned success per path, how many rows hold a pass key and whether two hold the same value, and per member the serial, community webhook and last successful push. The pass API contract, read off the live Mini-App on 15 Sep, is `src/lib/engine/eloyalty.ts`: two generations at once, v3 `/api/public/admin/…` addressed by customer id (points add and deduct, customer, events) and v1 `/api/external/v1/passes/{passTypeId}/{serial}/values/{Field}` addressed by pass type id plus serial (push and pass fields), so a membership holds all three (`eo_customer_id` is the key, `eo_pass_serial` a cache that goes stale on reinstall, `eo_pass_type_id`), every call site addresses through the one helper, a v1 404 means refresh the serial then retry, a points call with an unknown outcome reads the customer's events before any retry (no idempotency keys), a negative ledger amount is a deduct, and nothing ever sets a balance (the v1 `values/Points` field is refused). The creation call keeps `customerId` from the response. The first live call's checklist is in that file's header, including opening the pass to confirm the displayed balance moved after a v3 add (the assumption that v3 writes reach the pass display is the Mini-App's note, not an observation). The Evolve Omega program does not exist on the platform yet, so the Rewards page says passes aren't switched on and the push buttons are not offered. Inbound rewards and bookings flow back | ⚙️ Integrations |
| **Publishing** (per client) | Each client connects their own GoHighLevel sub-account on Settings → Publishing with a location-level Private Integration token plus their location ID. The scopes to tick are one list, `GHL_SCOPES` in `src/lib/engine/ghl-scopes.ts`, read by the Publishing page, the connection check (a 403 names the scope GoHighLevel refused) and this document: socialplanner/account.readonly, socialplanner/account.write, socialplanner/oauth.readonly, socialplanner/oauth.write, socialplanner/post.readonly, socialplanner/post.write, socialplanner/statistics.readonly, socialplanner/comment.readonly, socialplanner/comment.write, medias.readonly, medias.write, emails/builder.readonly, emails/builder.write, contacts.write, locations/customFields.write and locations/customFields.readonly (not called today: they create, and check for, the custom field the chatbot-lead path will write), conversations/message.write. Scopes are granted once, so the list is a superset of what the app calls now and within two releases, and the coach opens the setup to clients from Integrations ("Open Publishing setup to clients", off by default) only once it is final; until then a client's Settings says the setup isn't open yet. Saving validates the token against the sub-account and shows the real reason on failure (401 bad token, 403 missing scopes, wrong location). HelixOS lists the connected pages and profiles, auto-maps channels, sends every scheduled channel post to that client's Social Planner, edits a post the planner still holds as scheduled in place when it is re-scheduled or updated (same id, never a second copy), and syncs status back ("Check status"). The push runs after the action's response through Next's `after()`, which keeps the serverless invocation alive until it settles: a bare detached promise was frozen with the function the moment the composer returned, so the fetch never happened and nothing was written down (the reason no HelixOS post had ever reached the planner from Vercel). The Social Planner requires the posting user (`userId`), so the GHL user ID is required on Settings → Publishing and a push without one is refused there with the place to fix it, never sent. Every `postAt` in the database is a naive wall time in the member's own zone; the push converts it with `wallTimeToUtc` (src/lib/dates.ts) to the UTC instant, milliseconds and a Z, and the stagger is wall-time arithmetic (it used to shift through the browser's zone). What GoHighLevel's reply means is decided by the call that was made (`src/lib/engine/ghl-errors.ts`): a 422 is the request body refused and, on a post, names the refused field (user id, account, time, content); the location sentence is said only of the accounts call. Settings lists every account the sub-account returned, id and all, byte for byte; account ids are never parsed. Threads is a publishable channel (the Social Planner connects it through Instagram); a Facebook group is not (Meta removed group posting for every third-party tool in April 2024; `communityPostDetails` on a working call is a HighLevel Community, not a Facebook group); both sentences come from the channel map, once. Booked calls and new clients become contacts in the same sub-account (`contacts.write`) under one identity rule (`src/lib/engine/contact-sync.ts`): the first push needs an email or a phone (or, later, a chatbot id), GoHighLevel matches it under the sub-account's own duplicate setting and says whether it linked or created, and the id it returns is stored on the row (`contacts.ghl_contact_id`, `client_records.ghl_contact_id`) and is the target of every push after that, so no later push re-matches and a second contact cannot be created. A name alone is never pushed: the contact says "add an email or phone to sync this", a stage change that would push is refused with which one is missing, and the sync log says why. HelixOS holds the id, not a copy of the person. The coach's Integrations page has a contact sync replay per client (dry-run counts, one record first, then all, never automatic) and a per-client Disconnect; Disconnect on either page says what stops and that the token itself lives on until it is deleted in GoHighLevel. Anything token-shaped is redacted from an upstream body before it reaches a log or a note (`src/lib/engine/redact.ts`). Every integration push goes through `background()`, which hands the promise to Next's `after()`; a test refuses any other detached promise in request code (`detached-promises.test.ts`). `scripts/ghl-scope-probe.ts` is the scope-ordering experiment, run by a person with a throwaway token. What happened per channel is one rule (`src/lib/engine/channel-outcome.ts`) and one component (`src/components/channel-outcome.tsx`) read by the content card (the worst outcome wins, with a count), the composer right after scheduling (it asks again while any row is still sending) and the Distribute page (every version, with Check status per row and one click for all): six states with the word beside the light, Published only when the planner's readback said so (an accepted immediate post is Sending), Scheduled with the member's own wall time, Sending, Didn't send always with a reason, Copy and paste for a channel nobody can publish to (not a failure), and Lost track when a row is still sending twenty minutes past its time. A refusal the client can fix (no connection, no account chosen, no user id, no media for a story) is a failure, never "paste by hand". The planner's own error text is classified into a client sentence (`explainPlatformError`) and never stored as written; the unmapped case says plainly that we do not know. A 2xx that carries no id (seen live 15 Sep, two immediate posts published within the minute) is Accepted with the id pending: the id is found from the planner's own list (`posts/list`, same account, same text, since the request; `src/lib/engine/planner-match.ts`), which is also the guard against a second copy before any retry; two candidates in the window is ambiguous and refuses, so the row stays accepted with the id pending rather than adopting the wrong post; the list is asked for that account only, inside a window from ten minutes before the request to a day past the row's own time, so the answer never depends on how the planner orders a list longer than one page, and a window the planner says holds more than one page is refused, not guessed at (`scripts/ghl-list-order.ts` reports the live list's order, ids and timestamps only). Rows are checked least-recently-checked first, so the per-render cap is a rate, not a cliff. The first readback that says published logs the reply's shape (keys and whether postId looks like a Facebook id), never its values. The board and the Distribute page read back rows that are due as they open (`src/lib/planner-status.ts`: an accepted row every twenty seconds, any row with an id once an hour, the published ones included, so a post deleted in the planner reads Deleted), at most eight calls a render; the composer's banner never counts outcomes, the panel does. `scripts/sync-log.ts` prints the sync log for a window, one line per row. Verified against HighLevel's OpenAPI: `/social-media-posting/{locationId}/accounts`, `…/posts`, `…/posts/{id}` (GET and PUT), `…/posts/list` (read-only), `/contacts/upsert`. Integrations → Planner audit (coach only, read-only) lists planner posts HelixOS created and no longer tracks, from the sync log and the planner's scheduled list, and calls one a duplicate only when it is still scheduled with the same text and account as a tracked post; it never deletes | 📡 Social Planner |
...
| **Community Loyalty bot fields (Stage 1)** | HelixOS writes a client's business facts into their own Community Loyalty (uChat) bot, into the Book 'em Danno template's existing field names, by name, one call per client (`src/lib/engine/bot-fields.ts`, `src/lib/community-loyalty.ts`): the business name off the membership, the zone, the live offers as fact lines with their price and currency, a house constraints block, and the three qualifying questions the Offer carries (house defaults until written). The push is a named subset and asserts it touches none of the fields the client or the bot's agent writes (`calendar_id`, the appointment, the booked time), so a re-push after an unrelated edit leaves a chosen calendar and a booking exactly as they were. An offer, profile or workspace edit re-pushes when the payload changed; Re-sync on the Coach page pushes regardless. The request is the published UChat API's shape (`{ data: [{ name, value }] }`, every value a string, at most 20 a call, tested against the schema quoted from the spec), and a 200 is not a match: the fields are read back after each push and the record moves only when every sent value reads back. The token is the client's own, sealed at rest, opened only for the call; the record of the last push holds names and values and refuses anything shaped like a credential; the log holds names and a reason. One-way: bookings are read from GoHighLevel, never from the bot. Walk: `scripts/smoke-botfields.ts` against `scripts/mock-uchat.ts`. | 🤖 |
| **Provenance mark on AI drafts** | Four kinds of AI draft are stored the moment the model returns (a webinar section, a lead magnet, a doctrine post, group variants), before anyone has read a word. Each of those four tables carries `origin`: `ai_unreviewed` when the model's text is stored, `ai_accepted` on an explicit Accept, `edited` on any coach edit (an edit counts as review), `coach` for the coach's own words; rows from before the mark, and rule-composed text, stay null, never guessed and never flagged (`src/lib/engine/provenance.ts`). Where the draft is chosen, an unreviewed one carries "AI draft, not reviewed" beside an Accept for that one item; nothing accepts more than one. The gate stands on the action that sends content out, not on the page: the deck export, the lead magnet's Publish (the hosted page is public only once published; the public page shows nothing), scheduling or posting in the composer, one-click distribute, a post's or a version's status, each shows "N items are AI drafts you haven't reviewed", names them, and offers Review or Continue anyway; Continue anyway is logged in `review_confirms` with who and when and what it carried, and the deck route serves a deck with unread drafts only under such a confirm for exactly those drafts. Text a rule composed from what the coach chose (a principle's post, a rules variant, a ladder's rungs, a magnet's scaffold) is `rule`, never gated, so null means only a row from before the mark. The run sheet, the surface read aloud to a live room, marks an unreviewed script `[AI draft, not reviewed]` beside its section with a count at the top, and gates nothing, being a page. Walk: `scripts/smoke-provenance.ts`. | 🤖 |
| **Migrations, rehearsed and checked** | `scripts/check-migrations.ts` runs in the release gate: every table rebuild's copy statement is checked against the previous snapshot, so a migration that adds a column and rebuilds the same table (drizzle-kit selects the new column from the old table) is named before a walk seeds a database. `scripts/migrate-rehearsal.ts` runs the pending migrations against a local copy of a populated database (`DATABASE_URL=file:copy.db`, never a remote one) and reports row counts, offer prices and webinar numbers before and after; run it against a copy of production before a rebuild migration ships. | 🧪 |
| **Small and independent (F)** | A drafted example never becomes a value: "Start from the example" and the no-model path write nothing and show The Leaky Webinar's section beside the field, labelled as not yours, and a ✨ draft never copies example key points into the row. The address always names the section being edited; a step the URL names that does not exist is sent to a real one. A promise's timeframe recognises a single sitting. An offer's price carries its currency (`formatPrice`: "NZD $1,997", code beside symbol everywhere a price shows) and its container is chosen, never assumed. A source with no DOI (Gallup, the WHO, a government statistic) joins the shelf as unconfirmed with title, publisher, year and address, and a claim can be narrowed before it is confirmed. The Proof Bank's own add form asks the same permission tick as the Beliefs door. The schedule field says whose clock it is; post-event numbers are null until logged, so a funnel never reports a delivered event with no registrations. The acts' examples and soundbites are labelled as The Leaky Webinar's; the stage checklist is labelled as Evolve Omega's, with the build check named as the measure; the brand kit refuses muted-on-ground too and warns on the accent pairs with the ratio. | 🔧 |
| **Subject and brand kit** | Everything generated content is built from resolves through one subject (`src/lib/queries/subject.ts`, `src/lib/engine/subject.ts`): its name, Essence, approved proofs, stories, citable studies, offers and brand kit, plus the one set of known ids every check reads (`knownReferences`), so a reference whose row is gone never counts as a presence. The workspace owner is the only subject today; a later toggle passes a client record instead. The brand kit is one row per workspace, edited on Settings by the coach: six colours as bare hex, three faces and the required fallback face, banned colours; a kit whose ink cannot read on its ground at 4.5:1 or that uses a banned colour is refused with the pair named; the kit also names a placeholder colour (ink must read on it), the permitted names a script may introduce without a warning, which are never reported as the presenter, and whether the deck draws the price against the stack's total (`showPriceAnchor`, on by default: the control; off, the price stands on its own for a brand that forbids the comparison). Coaching copy that stated a runtime as fact now carries a slot the record fills (`{runtime}`, `{openingMinutes}`), and a drafted script is told the presenter and the session's minutes rather than left to invent them. | 🧭 |
| **Comment ladder handoff** (client) | A finished comment ladder is handed to the coach's Community Loyalty Rung Dripper (`docs` in the project: HelixOS_RUNG_DRIPPER.md; `src/lib/engine/rung-drip.ts`, `src/lib/rung-drip.ts`): HelixOS publishes the Facebook page and Instagram posts, Community Loyalty publishes Threads and comments the rungs onto the newest post on each. One POST to the coach's inbound webhook with `user_ns`, the raw post, rung 1 as `first_comment` (for the Threads post) and every rung from rung 1 as `rungs` joined on a `---` line; when Community Loyalty is the Facebook/Instagram publisher, `rungs` starts at rung 2 instead (encoded, tested against the flow's own splitter). The button is offered only when the gate passes, and the gate is said where the button would be: drip set up by Evolve Omega (the webhook URL is a credential, sealed, never rendered, never logged, with a scanner test), the post came from a ladder that clears its checklist, both posts read Published (the dripper pins the newest post on each), no other drip running for this coach (one at a time, the state lives on one contact), a Threads time at least fifteen minutes out or blank. The result is a row in the channel-outcome model, "Comments: handed to Community Loyalty at <time>" or "Comments: not handed off" with the reasons, and once a post is public while the handoff is refused the same row names the fallback (the rungs are ready to post by hand in Live posting hour, with a link to it), never a word that means posted: HelixOS is not told when a rung lands. While the handoff is on, the composer treats Threads as copy-only and the server refuses a Threads push, so nothing goes up twice. The composer has an optional first comment (`content_items.first_comment`), sent to the Social Planner as `followUpComment` on the Facebook page and Instagram posts only and kept on the item so a re-schedule carries it; the drip supplies every rung from rung 1, so on a ladder post with the handoff on the field is disabled with the reason beside it and the server refuses a first comment in every mode (`FIRST_COMMENT_LOCKED`); one click everywhere carries none for such a post. Whether GoHighLevel posts `followUpComment` on each platform is the observation the live test exists to produce. Danno installs the two values per client on the Coach page | 💬 Threads Publisher, Rung Dripper (Community Loyalty) |
| **Rewards** (client) | Earn Your Way: points are proof of work, tiers open doors. Every reward and prize is listed with what it asks for; an item with no booking link yet is never claimable, and the words distinguish a client who has met its tier and points ("You've earned this. It opens soon.") from one who has not ("Opening soon"); when any of the ladder has no link the page says so in one line. A claim is an instant unlock that reveals the item's own booking link; a refused claim says why and is not logged. Links live in `src/data/rewards-config.json` | 🏅 Rewards |
| **Community Pass** (Elite) | The client's own loyalty engine: pass settings, member leaderboard on the same 9 tiers, award points (pushes to their Community Loyalty webhook, which is the client's own credential with no auth on it: sealed at rest, opened only at the call, never shown again after saving, never in a note, kept out of the export; `scripts/seal-plain-secrets.ts` counts and, with --apply, seals rows saved before the column was sealed), daily hashtag post, 30-day curriculum to send | ⚙️ HelixOS Config, 📒 Points Ledger, 🏅 Community Tiers, 📆 30-Day Curriculum |
...
npx tsx scripts/smoke-socrates.ts    # Socrates Domain: lessons in order, library filters + own question, reframes split spoken/principle with the credit, script wizard to the fill step, the call sheet (both clipboard flavours) and the copy block, Conversations link
...
npx tsx scripts/smoke-deck.ts        # Deck v2: the opening contract is slides in the coach's order, a blank opening line is listed not shown, suggested pictures are counted; every deck-face.ts example kept off the exported .pptx's faces and a second currency refusing the export
npx tsx scripts/smoke-deletes.ts     # Deletes: the confirm (Cancel the default), back to the list with the line saying what went, the old address's plain not-found, and every detail route's not-found swept
...
npx tsx scripts/smoke-brain.ts       # The coach's brain, phase 1: the Knowledge Base Builder file imports as N entries, a priced answer needs eyes and is never bulk-accepted, approve/send/read-back both halves, one edit = one change, over-budget drops whole and lists, and the four refusals before a send (field not on the bot, agent not chosen, agent does not read the field, field holds text HelixOS did not write) (against scripts/mock-uchat.ts)
...
## The coach's brain: from their knowledge to their bot, phase 1

A coach gets their business knowledge into HelixOS with no friction, approves a plain-language **Bot Brief**, and their Community Loyalty bot updates itself; the coach never opens Community Loyalty. Phase 1 (`src/lib/engine/faq.ts`, `src/lib/actions/faq.ts`, `src/app/(app)/brain/page.tsx`, `src/lib/community-loyalty.ts`):

- **The FAQ store**, per client (`faq_entries`): the Knowledge Base Builder template's five fields (question, also asked, keywords, answer, category) plus `origin`. Every entry starts `ai_unreviewed` until the coach accepts it; an edit is a review (`edited`). Nothing reaches the bot otherwise.
- **Three intake doors into one parser**: an uploaded `.md`, `.txt` or `.docx`, pasted text, or the template's output pasted back from the coach's own AI. The format (`### Q:` / `Also asked:` / `Keywords:` / `Answer:` / `Category:`, with or without the template's bold markers) is parsed **deterministically, no model**; everything before the first `### Q:` is the header block, kept and never an entry. A `.pdf` is not read yet: the page says to paste its text. The fixture is `scripts/fixtures/knowledge-base.md`.
- **Danno's import**, read-only on Airtable: `npm run import:faq -- <owner email>` with `AIRTABLE_API_KEY`, `AIRTABLE_FAQ_BASE_ID` and `AIRTABLE_FAQ_TABLE` imports every Published row and reports the count the source returned (never assumed); Draft rows stay in Airtable; Airtable is never written; a re-run updates rather than doubles.
- **The Bot Brief**: one page, plain language. "Needs your eyes" (any answer naming a price, a guarantee, a result or a number) is pinned at the top and never bulk-accepted; the rest can be accepted at once. Who it speaks as, what it offers, what it knows (counts by category, expandable), what it won't do, when it steps aside, and what changed since the last sync (added / edited / removed). Every approval and every send is logged with who and when.
- **Route A, into a field of its own**: the approved answers are composed into **one bot field dedicated to the FAQ**, `ai_faq_cbf` by default, written only by HelixOS and overridable per member on the Coach page. The code refuses every Stage 1 field, every field the bot itself writes, and every `ai_product_*` field, because those hold the coach's own offer description and a push there would erase it. Four things must be true before anything is sent, each said in one sentence on the Brief: the field is one the FAQ may use; **the agent that answers questions is chosen** on the Coach page (blank never silently means "the first agent" — with more than one agent the Brief asks, naming them); **the field exists on the bot** (the API sets a field by name, it does not create one, so the Brief says "your bot needs the FAQ field added once" and sends nothing); and the field **holds nothing HelixOS did not write**, unless the coach ticks the overwrite override on the Coach page, which says first what it will replace. There is no type warning: Community Loyalty offers bot fields as Text only and never lets a type change, and the 20,000-character budget is the platform's own cap.
- **The coach's own bot**: a coach is a member too, so the Coach page opens with a **My bot** block carrying the same four integration fields a client's row carries (token, agent, FAQ field, overwrite) written to the coach's own membership. Without it a coach's own Brief could only tell them to ask their coach. The agent is never an `ai_agent_ns` typed from memory: once a token is saved the bot's agents are read with it (`GET /flow/ai-agents`) and **chosen by name** on the coach's own block and on every client row, storing the ns; with no token, or when the platform will not list them, the ns stays typeable so a bot the API cannot enumerate is never locked out. The Stage 1 business-facts push is not wired to a coach's own bot yet.
- **Push only what the agent reads**: before a push the target agent is read with `POST /flow/ai-agent-info` and the Brief shows only the fields its prompt carries. Community Loyalty's editor stores a field in a prompt as a chip that holds the field's **variable id** (`var_ns` on the bot-fields list, e.g. `f52594v17424617`) with the name only as its label, so the check resolves each field's id and looks for that, delimited so one id never matches inside another; `{name}` and `{{name}}` still count, the bare name in prose never does. The Brief, the pre-push check, the read-back and Stage 1's check share the one function (`agentReadsFields`), and one log line (`[faq.agent-reads]`) records the agent's raw prompt as the API returned it. The agent list (`GET /flow/ai-agents`) is cached a minute per bot for the Coach page and the Brief, which read it on every render; the check before a push and the read-back after it always read live, and a cached list naming an agent the bot no longer answers for is read again at once; a field it does not read gets the plain warning "your bot does not use this yet" and nothing is sent. After the push **both halves are read back** — the field's value matches and the token is still in the agent's prompt — and only then is the sync recorded as sent (`faq_syncs`, with the snapshot the next Brief diffs against).
- **One budget**: `FAQ_FIELD_BUDGET`, 20,000 characters, the platform's own cap (confirmed from the bot's error log and its prompt counters), named on the Brief. Entries are ranked by Times Asked when the source carried it, else recency; past the budget the lowest-ranked are dropped **whole** and listed, so a cut never lands mid-sentence in the bot's prompt.
- Not in phase 1: route B (the vector store), the MCP connector, drafting from Essence, and automatic Fathom intake. The live test on Danno's bot waits for his go-ahead. Walk: `scripts/smoke-brain.ts` against `scripts/mock-uchat.ts`.
...
returns real papers with title, authors, year, DOI as a working link and citation count, in OpenAlex's relevance order (never a citation sort, which surfaced the most-cited members of a loose match as if they were answers) and restricted to the field the model named (`fieldIdFor` maps it onto OpenAlex's 27 fields; a rejected filter is retried without and logged), every request logged under `[openalex] request` minus the key; a flag is on the terms' words rather than their phrases, the named author's absence is a flag, and a page where every result is flagged says the search found nothing and offers nothing to add; and the client
...
5. Log in as coach → Integrations: turn GoHighLevel on (no credential needed), enter the WalletPush app key. Send a client their invite link. Each client connects their own GoHighLevel sub-account from Settings → Publishing.
