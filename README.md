...
npx tsx scripts/smoke-ghl.ts       # Client's own Private Integration token → 401 / 403 / wrong-location reasons → accounts → channel map → schedule → re-schedule edits in place → status sync, against scripts/mock-ghl.ts
