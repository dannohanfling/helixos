...
async function wipeDemo(): Promise<void> {
  ...
  const byWs = [schema.tasks, schema.contentItems, schema.contacts, schema.dailyLogs, schema.pointsLedger, schema.pathwayProgress, schema.curriculumProgress, schema.goals, schema.rewardClaims, schema.memberships] as const;
...
export async function seedDemo(): Promise<void> {
  ...
  await db.insert(schema.memberships).values([
    ...
    { id: newId(), workspaceId: wsId, userId: clientId, role: "client", businessName: "Torres Nutrition Coaching", bigPromise: "I help busy moms drop 15 lbs in 90 days without giving up wine or weekends.", startedAt: addDays(today, -24) },
