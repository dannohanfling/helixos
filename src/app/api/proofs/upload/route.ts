...
import { admissionFor, reapOrphans } from "@/lib/queries/proof-attachments";
  ...
  try {
    const json = await handleUpload({
      ...
      onBeforeGenerateToken: async (pathname, clientPayload) => {
        ...
        // The previous failed upload on this proof, if any, leaves the store before the next one starts.
        await reapOrphans(v.workspace.id, proof.id);
