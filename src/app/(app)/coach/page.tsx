              ...
              {rows.map((r) => (
                <li key={r.m.id} className="flex flex-wrap items-center gap-2 py-2 text-sm">
                  ...
                  <form action={setMemberPassAction} className="flex flex-1 flex-wrap items-center gap-1">
                    ...
                    <input className="field w-44 py-1 text-xs" name="faqBotField" placeholder="FAQ bot field (blank = default)" defaultValue={r.m.faqBotField ?? ""} data-testid="faq-bot-field" title="The one longtext bot field the approved FAQ answers are composed into. Blank means the Product & Service Information field the Booking Agent reads." />
