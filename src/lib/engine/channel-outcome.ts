...
export type OutcomeState = "published" | "scheduled" | "sending" | "failed" | "manual" | "unknown";
export const OUTCOME_WORD: Record<OutcomeState, string> = { published: "Published", scheduled: "Scheduled", sending: "Sending", failed: "Didn't send", manual: "Copy and paste", unknown: "Lost track" };
...
export function outcomeOf(row: OutcomeRow, now: Now, cutoffMinutes = SENDING_CUTOFF_MINUTES): ChannelOutcome {
  ...
  const base = { id: row.id, channel: row.channel, label, canCheck: Boolean(row.externalId) };
  ...
  if (ext === "failed" || ext === "deleted" || (ext === "manual" && fixableSkip(row.externalError))) return { ...base, state: "failed", word: OUTCOME_WORD.failed, when: null, reason: row.externalError?.trim() || UNMAPPED_FAILURE };
  // Accepted or still on its way: scheduled while its time is ahead; sending until the cutoff after it; then lost track.
  const accepted = ext === "scheduled" || ext === "in_progress" || ext === "pending" || ext === "in_review" || ext === "notification_sent";
  ...
  return { ...base, state: "sending", word: OUTCOME_WORD.sending, when: row.postAt && !accepted ? whenText(row.postAt, now.today) : null, reason: null };
}

/** The rows a post has outcomes for: drafts and skipped versions are not outcomes. */
export function outcomesFor(rows: OutcomeRow[], now: Now): ChannelOutcome[] {
  return rows.filter((r) => r.status !== "draft" && r.status !== "skipped").map((r) => outcomeOf(r, now));
}

const SEVERITY: OutcomeState[] = ["failed", "unknown", "sending", "scheduled", "published", "manual"];
...
export function summarize(outcomes: ChannelOutcome[]): OutcomeSummary {
  const counts: Record<OutcomeState, number> = { published: 0, scheduled: 0, sending: 0, failed: 0, manual: 0, unknown: 0 };
  for (const o of outcomes) counts[o.state]++;
  const total = outcomes.length - counts.manual;
