...
export function primaryTarget(m: { primary: "page" | "pdf" | "file"; formats: MagnetFormats; slug: string; pdfKey: string | null; fileKey: string | null; publishedAt: string | null }, publicUrlFor: (key: string) => string | null): string | null {
  ...
  // The hosted page is a target only once the client published it: the tracked link never sends a reader to a page that is not there.
  if (m.formats.page && m.publishedAt) return `/m/${m.slug}`;
