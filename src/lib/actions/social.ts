...
import { connectionFor, refreshAccounts, upsertConnection } from "@/lib/ghl";
import { recordReadback, reconcileLostId } from "@/lib/planner-status";
...
export async function syncPostStatusAction(formData: FormData): Promise<void> {
  ...
  const conn = await connectionFor(userId);
  if (!conn) return;
  if (!variant.externalId) await reconcileLostId(variant, conn);
  else await recordReadback(variant, conn);
  refresh();
}
...
export async function checkAllPostStatusAction(formData: FormData): Promise<void> {
  ...
  for (const variant of variants) {
    if (variant.externalId) await recordReadback(variant, conn);
    else if (variant.externalStatus === "accepted") await reconcileLostId(variant, conn);
  }
  refresh();
}
