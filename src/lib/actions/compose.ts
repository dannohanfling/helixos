...
export async function polishTargetsAction(input: { title: string; hook: string; body: string; cta?: string; hasCta: boolean; targets: { key: string; channel: string; groupId: string; body: string }[] }): Promise<Record<string, { body: string; subject?: string }>> {
  ...
  const lines = input.targets.map((t) => {
    ...
    const rules = g ? `Facebook group "${g.name}". Mission: ${g.mission ?? ""}. Admin values: ${g.adminValues ?? ""}. Rules: ${g.rules ?? ""}. Norms: ${g.postingNorms ?? ""}. ${g.kind === "own" ? "This is the author's own group: full CTA welcome." : "Guest post: value first, no links, no pitch."} Max ${spec?.maxChars} chars. Links: ${readRules(g).noLinks ? "none" : spec?.links}.` : `${spec?.label}. ${spec?.tone} Max ${spec?.maxChars} chars. Links: ${spec?.links}.`;
    return `- ${t.key}: ${rules}\n  Current draft:\n${t.body}`;
  });
